...
function recursionUpdate(target, options) {
    ...
    if (target.$$parent && target.$$name) {
        let defaultTarget = Object.assign(new target.constructor(target.$$defaultTarget, target.$$name, target.$$parent), target);
        target.$$parent[target.$$name] = observable(defaultTarget, options, target.$$name, target.$$parent);
